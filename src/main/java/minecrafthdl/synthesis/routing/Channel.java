    ...
    public void repeatNets(Circuit channel, Net n) {
            ...
            if (p.top) {
                if (n.trackZ() > 10) {
                    for (int z = n.trackZ() - 10; z > -1; z -= 10){
                        channel.setBlock(p.xPos(), 0, z, Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.NORTH));
                    }
                }

                if (p.xPos() > n.x_min) {
                    channel.setBlock(p.xPos() - 1, 2, n.trackZ(), Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.EAST));
                }

                if (p.xPos() < n.x_max) {
                    channel.setBlock(p.xPos() + 1, 2, n.trackZ(), Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.WEST));
                }
            } else {
        ...
        if (n.x_max - n.x_min > 19) {
            for (int x = n.x_min + 2; x < n.x_max - 2; x += 14) {
                ...
                if (!(!n.outpath && n.out_partner != null)) {
                    if (x < n.top_pin.xPos())
                        channel.setBlock(x, 2, n.trackZ(), Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.EAST));
                    if (x > n.top_pin.xPos())
                        channel.setBlock(x, 2, n.trackZ(), Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.WEST));
                } else {
                    channel.setBlock(x, 2, n.trackZ(), Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.EAST));
                }
            }
        ...
        if (n.isOutpath()){
            if (n.out_partner.trackZ() - n.trackZ() > 14) {
                for (int z = n.trackZ() + 3; z < n.out_partner.trackZ() - 2; z += 13){
                    channel.setBlock(n.x_max, 0, z, Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.NORTH));
                }
            }

            channel.setBlock(n.x_max - 1, 2, n.trackZ(), Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.WEST));
        }

        if (!n.isOutpath() && n.out_partner != null){
            channel.setBlock(n.x_max - 1, 2, n.trackZ(), Blocks.REPEATER.defaultBlockState().setValue(BlockStateProperties.FACING, Direction.EAST));
